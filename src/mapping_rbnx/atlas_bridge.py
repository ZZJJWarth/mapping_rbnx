...
from robonix_api import Service, Ok, Err, ATLAS  # noqa: E402

# Map operation gRPC + MCP stubs are generated by `rbnx codegen --mcp`.
...
def _enabled_sensors(cfg: dict) -> dict:
    ...
    if not isinstance(sensors, dict) or not sensors:
        raise RuntimeError(
            ...
            "Supported keys: "
            + ", ".join([*(k for k, _, _ in _SENSOR_CONTRACTS), "rgbd"])
        )
    out = {}
...
def save_map(req: McpSaveMapReq) -> McpSaveMapResp:
    """Snapshot the current SLAM map to a named map directory."""
...
def load_map(req: McpLoadMapReq) -> McpLoadMapResp:
    """Load a saved map for mapping or localization mode."""
...
def pose_estimate(req: McpPoseReq) -> McpPoseResp:
    """Seed SLAM localization with an approximate map-frame pose."""
...
def switch_mode(req: McpSwitchReq) -> McpSwitchResp:
    """Switch the running SLAM session between mapping and localization."""
